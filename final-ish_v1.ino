...
const uint16_t max_stored = 120; //20 min worth of samples if you only keep every 4th point
...
void loop() {
  ...
  if (millis() - last_time > PERIOD) {
    ...
    if ((sig_moving_avg_index - 1) % 4 == 0) { // if it's fourth point, store for display!
      stored_ind++;
      if (stored_ind > max_stored) { // once you're past max number of samples that can be stored...
        ...
        if (sig_moving_avg_index < max_BT_recalc_num_samples) {
          stored_sig[max_stored - 1] = test[sig_moving_avg_index - 1];
        }
        else {

          stored_sig[max_stored - 1] = test[max_BT_recalc_num_samples - 1]; //recall your latest signal pt is at index = max_BT(...)
        }
      }
      else if ((sig_moving_avg_index - 1) < max_BT_recalc_num_samples) { // if you're w/in max_stored (ie passed first if) AND bt_recalc...
        // recall you cannot compare stored_ind and bt_recalc bc stored_ind is 1/4 of sig_moving_avg_index
        ...
        stored_sig[stored_ind] = test[max_BT_recalc_num_samples - 1]; //your signal index is described by max_BT(...)
      }
    }
    ...
    if (button3 == LOW) {
      if (state == 0) { // if on history screen...
        ...
        change_graph = true;
        if (stored_ind >= max_stored) {
          int stop_index = max_stored - 1;
          int start_index = stop_index - (int)((sig_moving_avg_index - 240 * (int) (sig_moving_avg_index / 240)) / 4);
          int left_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 2) * 60;
          Graph_Setup(tft, left_rail, stored_sig[start_index], 50, 290, 390, 260, 60 * (int) (sig_moving_avg_index / 240), 60 * (int) (sig_moving_avg_index / 240) + 60, 10, -100, 200, 50, "Contractions", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
        }
        else {
          int start_index = 60 * (int) ((stored_ind - 1) / 60);
          Graph_Setup(tft, start_index, stored_sig[start_index], 50, 290, 390, 260, 60 * (int) (sig_moving_avg_index / 240), 60 * (int) (sig_moving_avg_index / 240) + 60, 10, -100, 200, 50, "Contractions", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
        }
      } else if (state == 1) { // if on graph screen...
        tft.fillScreen(BLACK);
        state = 2; //go to history graph screen...
        redraw = true;
        history_on = false;
      } else if (state == 2) { // if on history graph screen...
        tft.fillScreen(BLACK);
    ...
    if (state == 1 ) {
      ...
      if (history_on == false) {
        ...
        if (sig_moving_avg_index < 240) { // if less than 10 min, plot up to current time

          if (change_graph == true) { // when you flip to this screen, plot the points you missed when you weren't here

            for (plot_x = 0; plot_x <= stored_ind; plot_x++) {
              plot_y = stored_sig[plot_x];
              //       inputs, gx, gy, w,  h,    xlo,xhi,xinc,ylo,yhi,yinc
              Graph(tft, plot_x, plot_y, 50, 290, 390, 260, 0, 60, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            }
            change_graph = false;
          }
          else {
            if ((sig_moving_avg_index - 1) % 4 == 0) {
              plot_y = stored_sig[stored_ind];
              plot_x = (sig_moving_avg_index - 1) / 4;
              //       inputs, gx, gy, w,  h,    xlo,xhi,xinc,ylo,yhi,yinc
              Graph(tft, plot_x, plot_y, 50, 290, 390, 260, 0, 60, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            }
          }
        } // end if less than 10 min


        else {// if past 10 min...

          // first set start & stop indices. These tell you where in stored_sig you are
          int start_index = 0;
          int stop_index = 0;
          if (stored_ind > max_stored) { //after you pass max_stored number of samples, you can no longer use stored_ind as the index; the vector is being rewritten (most recent point is always last point)
            stop_index = max_stored - 1;
            start_index = stop_index - (int)((sig_moving_avg_index - 240 * (int) (sig_moving_avg_index / 240)) / 4);
          }
          else { //if you are still within max_stored number of samples you are still adding to the end of the sig_stored vector
            start_index = 60 * (int) ((stored_ind - 1) / 60);
            stop_index = stored_ind - 1;
          }

          // every 10 minutes you need to go to next 10 minutes!
          if ((sig_moving_avg_index - 1) % 240 == 0) {
            tft.fillScreen(BLACK);
            if (stored_ind >= max_stored) {
              int left_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 2) * 60;
              Graph_Setup(tft, left_rail, stored_sig[start_index], 50, 290, 390, 260, 60 * (int) (sig_moving_avg_index / 240), 60 * (int) (sig_moving_avg_index / 240) + 60, 10, -100, 200, 50, "Contractions", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
          ...
          if (change_graph == true) {
            int left_rail;
            int right_rail;
            if (stored_ind >= max_stored) {
              left_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 2) * 60;
              right_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 1) * 60;
            }
            else {
              left_rail = start_index;
              right_rail = start_index + 60;
            }

            for (plot_x = start_index; plot_x <= stop_index; plot_x++) {
              plot_y = stored_sig[plot_x];
              int actual_plot_x;
              if (stored_ind >= max_stored) {
                actual_plot_x = left_rail + plot_x - start_index;
              }
              else {
                actual_plot_x = plot_x;
              }
              //       inputs, gx, gy, w,  h,    xlo,xhi,xinc,ylo,yhi,yinc
              Graph(tft, actual_plot_x, plot_y, 50, 290, 390, 260, left_rail, right_rail, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
              //Graph(tft, plot_x, plot_y, 50, 290, 390, 260, 0, 60, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            }
            change_graph = false;
          }
            ...
            if ((sig_moving_avg_index - 1) % 4 == 0) {
              ...
              int right_rail;
              if (stored_ind >= max_stored) {
                plot_y = stored_sig[max_stored - 1];
                left_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 2) * 60;
                right_rail = stop_index + 1 + ((int)(sig_moving_avg_index) / 240 - 1) * 60;
              }
              else {
              ...
              //       inputs,           gx, gy, w,  h,    xlo,           xhi,            xinc,ylo,yhi,yinc
              Graph(tft, plot_x, plot_y, 50, 290, 390, 260, left_rail, right_rail, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
              //Graph(tft, plot_x, plot_y, 50, 290, 390, 260, 0,60, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            }
        ...
        if (button1 == LOW || button2 == LOW ) { //if button is pressed AND you don't exceed max index of stored signal
          history_on = true;
        }
      }
      else {
        ...
        int curr_start_rail = left_rail;
        int curr_end_rail = right_rail;
        int curr_start_index = start_index;
        int curr_stop_index = stop_index;

        


        if (button1 == LOW) { //flip button to towards future
          if (curr_stop_index >= (max_stored - 60)) {
            history_on = false;
          }
          else {
            curr_stop_index = curr_stop_index + 60;
            curr_start_index = curr_start_index + 60;
            curr_start_rail = curr_start_rail + 60;
            curr_end_rail = curr_end_rail + 60;
            tft.fillScreen(BLACK);
        Graph_Setup(tft, curr_start_rail, stored_sig[curr_start_index], 50, 290, 390, 260, curr_start_rail, curr_end_rail, 10, -100, 200, 50, "Contractions", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            for (int pat = curr_start_index; pat<curr_stop_index; pat++){
              int plot_y = stored_sig[pat];
              int plot_x = curr_start_rail + pat - curr_start_index;
              Graph(tft, plot_x, plot_y, 50, 290, 390, 260, left_rail, right_rail, 10, -100, 200, 50, "CONTRACTIONS", " Time [s]", "UC [arb. units]", DKBLUE, RED, GREEN, WHITE, BLACK);
            }
          }

        }
        if (button2 == LOW) { //flip button to go backwards
          if (curr_start_index >= 60) {
            curr_stop_index = curr_stop_index - 60;
            curr_start_index = curr_start_index - 60;
            curr_start_rail = curr_start_rail - 60;
            curr_end_rail = curr_end_rail - 60;

                    Serial.print(curr_start_rail);
              Serial.print("\t");
              Serial.print(curr_end_rail);
              Serial.print("\t");
              Serial.print(curr_start_index);
              Serial.print("\t");
              Serial.print(curr_stop_index);
              Serial.print("\t");
              Serial.print(sig_moving_avg_index);
              Serial.println();
                        tft.fillScreen(BLACK);
